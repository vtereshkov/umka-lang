...
void
storageInit(Storage *storage)
{
	storage->first = storage->last = NULL;
}

void
storageFree(Storage *storage)
{
...
char *
storageAdd(Storage *storage, int size)
{
...
char *
storageAddStr(Storage *storage, int len)
{
...
static void *
moduleLoadImplLib(const char *path)
{
#ifdef UMKA_EXT_LIBS
#ifdef _WIN32
...
static void *
moduleLoadImplLibFunc(void *lib, const char *name)
{
#ifdef UMKA_EXT_LIBS
#ifdef _WIN32
...
void
moduleInit(Modules *modules, bool implLibsEnabled, Error *error)
{
...
void
moduleNameFromPath(Modules *modules, const char *path, char *folder, char *name, int size)
{
...
int
moduleFind(Modules *modules, const char *path)
{
	unsigned int pathHash = hash(path);
	for (int i = 0; i < modules->numModules; i++)
		if (modules->module[i]->pathHash == pathHash &&
		    strcmp(modules->module[i]->path, path) == 0)
			return i;
	return -1;
}

int
moduleFindImported(Modules *modules, Blocks *blocks, const char *alias)
{
	for (int i = 0; i < modules->numModules; i++) {
...
int
moduleAdd(Modules *modules, const char *path)
{
	...
	for (int i = 0; name[i]; i++) {
		if (name[i] == ' ' || name[i] == '\t') {
			modules->error->handler(
			    modules->error->context, "Module name cannot contain spaces or tabs");
		}
	}
	...
	if (modules->implLibsEnabled) {
		...
		sprintf(libPath, "%s%s%s%s.umi", pathPrefix, module->folder, module->name,
		    moduleImplLibSuffix());
...
char *
moduleFindSource(Modules *modules, const char *path)
{
	unsigned int pathHash = hash(path);
	for (int i = 0; i < modules->numModuleSources; i++)
		if (modules->moduleSource[i]->pathHash == pathHash &&
		    strcmp(modules->moduleSource[i]->path, path) == 0)
			return modules->moduleSource[i]->source;
	return NULL;
}

void
moduleAddSource(Modules *modules, const char *path, const char *source)
{
...
void *
moduleGetImplLibFunc(Module *module, const char *name)
{
	if (module->implLib)
...
char *
moduleCurFolder(char *buf, int size)
{
#ifdef _WIN32
...
bool
modulePathIsAbsolute(const char *path)
{
...
void
moduleAssertRegularizePath(
    Modules *modules, const char *path, const char *curFolder, char *regularizedPath, int size)
{
...
void
blocksInit(Blocks *blocks, Error *error)
{
	blocks->numBlocks = 0;
	blocks->top = -1;
	blocks->module = -1;
...
void
blocksFree(Blocks *blocks)
{
}

void
blocksEnter(Blocks *blocks, struct tagIdent *fn)
{
...
void
blocksReenter(Blocks *blocks)
{
	blocks->top++;
}

int
blocksCurrent(Blocks *blocks)
{
...
void
externalInit(Externals *externals)
{
	externals->first = externals->last = NULL;
}

void
externalFree(Externals *externals)
{
...
External *
externalFind(Externals *externals, const char *name)
{
...
External *
externalAdd(Externals *externals, const char *name, void *entry)
{
