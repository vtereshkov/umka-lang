...
bool identIsMain(Ident *ident)
{
    return strcmp(ident->name, "main") == 0 && ident->kind == IDENT_CONST &&
           ident->type->kind == TYPE_FN && !ident->type->sig.isMethod && ident->type->sig.numParams == 1 && (ident->type->sig.resultType->kind == TYPE_VOID || ident->type->sig.resultType->kind == TYPE_INT);  // A dummy "__upvalues" is the only parameter
}
